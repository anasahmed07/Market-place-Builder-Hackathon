...
export default function (props: ProductPriceProps) {
    if (!props.discount) {
        return (
            <span className={props.pdp?`${satoshi.className} my-9 text-2xl font-bold text-gray-900`:`${satoshi.className} text-lg font-bold text-gray-900`}>${ props.price }</span>
        )
    }
    else {
        return (
            <div className={props.pdp?`${satoshi.className} my-4 inline-flex items-center space-x-2`:`inline-flex items-center space-x-2`}>
